...
src/                         全 Python コード
  ...
  # ── Paper 2: Contextual Mode Resolver ──
  lod_persona.py           LOD ペルソナ生成(CSP / SAT-UNSAT; --variant v1|grid)
  lod2_cluster.py          LOD2 戦略分岐を生成し cos クラスタリング
  merge_paper2_data.py     国別 event DB 統合(LOD0 事象 / LOD1 解釈を物理分離; --country us|uk --variant v1|grid)
  recover_gemini_jsonl.py  破損 Gemini 出力(txt)を決定論的に復旧(捏造なし)
  generate_claude_observer.py 第2観測者 Claude の grid 解釈を出力(Gemini欠落共同体を独立判定で2観測者化)
...
- **LOD アーキテクチャ**([`LOD_ARCHITECTURE.md`](LOD_ARCHITECTURE.md)): 曝露構造(LOD 0, 数理)からペルソナ(LOD 3, 解釈)までを解像度の階層として扱い、4公理(Anchor Preservation / Non-overwrite / Projection Consistency / Provenance)を制約とする CSP として定式化。[`lod_persona.py`](src/lod_persona.py) は Projection Consistency と Axiom 違反で SAT/UNSAT を判定(`arch -arm64 python3.12 src/lod_persona.py ... --variant grid`)。
