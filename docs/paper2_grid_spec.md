...
同性婚(Section 28 撤廃〜合法化)/ スコットランド独立投票

→ **US 8×12 = 96 セル / UK 9×12 = 108 セル**(欠損ゼロ目標)
...
```json
{
  ...
  "source_urls": ["https://..."], "confidence": "high"
}
```
...
- `rationale` はその community にとっての**作用の質**を1-2文で(なぜその mode か)。
- UK は `effect_emphasis`(効いた作用ベクトル軸のタグ)を付ける。
- **数値スコア(code_factors / salience_multiplier 等)は要求しない**(Honest Structuralism:rationale で足りる。数値化は別タスク)。
...
> 数値スコアは不要。mode と rationale(UK は effect_emphasis も)のみ。
>
...
python3 merge_paper2_data.py --country us     # consensus/interpretations/disagreements 再生成
python3 cmr_matrix.py  --country us           # 欠損ゼロの mode 変換マトリクス
python3 cmr_compare.py --country us --birth_year 1985
python3 make_paper2_figures.py                # Fig2/Fig3 を欠損なしで再描画
```
