...
4. `python3 src/ess_acquire.py` → 必要列を slim parquet 化(`data/ess/ess_slim.parquet`)。
...
- **step1(本命):Europe-wide, `freehms` 主検証** — high religiosity vs low / high-edu vs low / country clusters
  で high-flat / transition を見る → **US 同性婚(GSS)と並置**(目玉図)。`src/ess_core_validation.py`。
...
- 取得:`src/ess_acquire.py` / 設計:`src/ess_segments.py` / step1:`src/ess_core_validation.py`
