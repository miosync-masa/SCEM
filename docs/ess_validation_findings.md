...
環境変数(リポジトリに焼かない)。`src/adapters/ess/ess_acquire.py` に DOI と取得を実装。
...
**Secular HiEdu Urban(Coastal 類似) vs Religious LowEdu(Bible Belt 類似)**。`src/adapters/ess/ess_core_validation.py`。
...
`src/adapters/ess/ess_valuepack.py`。Europe-wide(country FE)で消えた freehms 移行が、どのクラスタに在るか分解。
...
`src/adapters/ess/ess_southern_country.py`。Southern クラスタの z=11.78 が複数国一貫か1国偏りかを **事前固定基準**で判定
...
`src/adapters/ess/ess_effective_year.py`。「**事象の着弾年(effective_year)が国で違う → 移行段階が違う → スロープ可視性が違う**」
...
> 制度年は **出典付きで検証済**:`docs/ess_legal_coding.md`(Wikipedia *Recognition of same-sex unions in Europe*=
> 各国国内立法を脚注引用 + ILGA-Europe で横断確認、アンカー国は国内法明記 ES Ley13/2005・PT Lei9/2010・
> IT Law76/2016・GR Law4356/2015 等)。当初コーディングは全件裏取りされた(WebFetch 要約の「—」は抽出漏れ)。
> 制度年 = 事象の着弾年であって個人の mode ではない(混同しない)。最終投稿前に ILGA Rainbow Map 最新版で 1 国ずつ再突合。
...
- 操作化(承認年 vs 同性婚年)は **感度テストで頑健**:婚姻年版でも同符号(スロープ z +0.34 / 変化点 **+0.67**)。
  GR/CY 等 2 波で period 統制弱。**国 n=30 だが exploratory → 確証と書かない。**
...
`src/adapters/ess/ess_overlay.py`(UK GB N=9,260)。UKグリッド事前 resolved_mode × ESS UK 観測。
