...
**同年生まれ・別 Community(grid 母数)**:1985年生まれを固定し、grid の全 community で LOD0 を解決 → Paper 1 エンジンに投入(`cmr_compare.py --variant grid`)。指紋(PASSIVE/ACTIVE/REFRAME 密度)が community で割れ、**Contextual Divergence Index = US 0.321 / UK 0.338**。

![US fingerprints](../figures/fig_p2_fingerprints_us_1985_grid.png)
![UK fingerprints](../figures/fig_p2_fingerprints_uk_1985_grid.png)

- US:Coastal Liberal / Suburban MC は **REFRAME 支配**(MFR 92% / 83%)、Bible Belt・Rust Belt・Latino・Rural は **ACTIVE 支配**(MFR 17%)。同じ 1985 生まれでも前提で指紋が反転。
