...
**的中率(grid REFRAME/ACTIVE の 18 セル)= 線形 11/18 (61%) / 変化点 12/18 (67%)。** 事象別(変化点):

| 事象 | grid予測の的中 | 外したセル |
|---|---|---|
| **同性婚** | **5/6** | Suburban MC(予測REFRAME→実transition +7.1) |
| **中絶** | **5/6** | **Coastal Liberal**(予測REFRAME→実transition +4.7) |
| **銃規制** | **2/6** | grid ACTIVE 予測が軒並み外れ(GSS は全 flat) |

2×2(変化点):grid ACTIVE→transition **8** / →flat 4 ; grid REFRAME→flat **4** / →transition 2。
...
3. **銃規制が系統的に外れる(重要)**:grid は銃を ACTIVE(動員・態度表明)と解決したが、**GSS の出生年
   *態度* は全共同体 flat**。→ **ACTIVE(動員/決断強制)は必ずしも「出生年態度勾配」を生まない**。
   操作化「ACTIVE→cohort態度transition」自体が不完全。これは方法論的な気づき(ACTIVE は行為/スタンスの
   モードで、態度のコホート自由化とは別物でありうる)。Discussion 候補。
4. **中絶×Coastal:グリッドが外し、人間の直感が当てた**。C で真道さまが「中絶は Coastal 急かも」と予言 →
   GSS は transition(+4.7)。だがグリッドは REFRAME(flat)と予測 → **グリッド(LLM)が外し**。Dobbs(2022)が
   Coastal で中絶を再活性化した可能性。LLM 事前予測も完璧でない、を正直に記録。

**overlay の正味**:(ii)は「issues differ」を概ね棄却し、**SSM・中絶でグリッド事前予測どおりにモードが解決**
(= CMR 本丸の予測対応)。一方 **銃規制の系統失敗**が「ACTIVE→態度勾配」操作化の限界を露呈し、**Coastal×中絶**
で LLM グリッドの誤予測も判明。**「確証」でなく「予測対応(modest, n=18, 事象構造あり)+ 2つの正直な綻び」**
として報告するのが誠実。これでも *issues differ* 凡庸説より遥かに強い。
