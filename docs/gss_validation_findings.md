...
| | marsame 非欠損 N | コホート勾配の形 |
|---|---|---|
| **Coastal Liberal** | **116(薄い)** | **平坦高位**:全出生年で賛成 ~83–100%。コホート差ほぼ無し |
| **Bible Belt** | **1,166(頑健)** | **明瞭な勾配**:1925–29生 6% → 1965–69生 33% → 1985–89生 42% → 1990–94生 66% |

### 2.1 これは CMR 予測の向きと整合(ただし記述・検定は未実装)
- **Coastal = REFRAME 署名**:出生年に関わらず賛成が高位で平坦 = 「基準値が既に書き換わった共同体」。
  世代を追っても動く余地が小さい(天井近く)。REFRAME 予測共同体が baseline で先行している像。
- **Bible Belt = ACTIVE/争点の前線**:出生年で賛成率が大きく動く = 共同体内で世代間にコントラスト。
  「議論が割れ・態度が動く」前線がここにある。spec の polarization 検証が効くのはこちら。

> ⚠️ **これは記述のみ**。「早期立ち上がり(shift)」「分散拡大(polarization)」の**検定統計量は未実装**
> (spec 6.3/6.4 未決)。曲線の形が予測と整合する、という以上の主張はまだしない(後付け防止 §5.2)。
...
1. **Coastal の N 薄問題(構築判断)**:`degree==4`(院卒のみ)で N=116。`degree>=3`(学士+)に緩めると
   厚くなるが定義が変わる。緩める/地理を広げる/Coastal を「平坦高位の確認」に留め主力対比を別ペアにする、
   のいずれか。**spec §4.1 の定義変更にあたるので環の判断**。
