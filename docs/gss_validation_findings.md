...
### 2.5 主検定の実行結果(`src/adapters/gss/gss_interaction.py`)— **正直な結果:方向◯・有意✗**
...
### 2.6 徳用パック(`src/adapters/gss/gss_valuepack.py`)— 4事象 × 主力6共同体。**正直な結末:事象で割れた**
...
### 2.7 overlay:Paper 2 事前予測 × GSS 観測(`src/adapters/gss/gss_overlay.py`)— (ii)を「整合」から「予測対応」へ
...
- **GUNIMP 追撃は不要**(`src/adapters/gss/gss_overlay.py` 確認済:GUNIMP は 2波[1976,1984]・GUNFIRM は 1波[1984]、全て
...
```bash
python3 src/adapters/gss/gss_acquire.py          # GSS 取得 → data/gss/gss_slim.parquet(生データは .gitignore)
python3 src/adapters/gss/gss_core_contrast.py    # コア対比(記述)→ figures/ + data/gss_results/
```
